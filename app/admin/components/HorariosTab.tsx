            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-2">
                ...
                <button onClick={() => { if (nuevoDia && !diasSeleccionados.includes(nuevoDia)) setDiasSeleccionados([...diasSeleccionados, nuevoDia]); setNuevoDia(""); }}
                  className="p-3 bg-zinc-100 dark:bg-zinc-800 rounded-xl hover:bg-tekila-pink transition-all"><Plus size={18} /></button>
              </div>
                ...
                {lista.map((h: any) => (
                  <div key={h.id} className="py-3 flex justify-between items-center group">
                    <div className="flex items-center gap-3">
                      {view === 'libres' && (
                        <button
                          ...
                          className="text-zinc-300 hover:text-tekila-pink transition-colors"
                        >
                          {seleccionados.includes(h.id) ? <CheckSquare size={18} className="text-tekila-pink" /> : <Square size={18} />}
                        </button>
                      )}
                    ...
                    <div className="flex gap-2">
                      {view === 'libres' ? (
                        <>
                          <button onClick={() => { setEditId(h.id); setNuevoHorario(h.dia_hora.slice(0, 16)); window.scrollTo({ top: 0, behavior: 'smooth' }); }} className="p-2 text-zinc-300 hover:text-tekila-pink"><Edit3 size={16} /></button>
