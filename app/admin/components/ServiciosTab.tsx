    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="flex justify-center mb-6">
        <div className="flex bg-zinc-100 dark:bg-zinc-900 p-1.5 rounded-2xl w-full max-w-xs shadow-inner">
          <button onClick={() => setMobileView('lista')} className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl text-[0.625rem] uppercase font-bold tracking-widest transition-all ${mobileView === 'lista' ? 'bg-white dark:bg-zinc-800 text-tekila-pink shadow-md' : 'text-zinc-400'}`}>
            <List size={14} /> Lista
          </button>
          <button onClick={() => setMobileView('formulario')} className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl text-[0.625rem] uppercase font-bold tracking-widest transition-all ${mobileView === 'formulario' ? 'bg-white dark:bg-zinc-800 text-tekila-pink shadow-md' : 'text-zinc-400'}`}>
      ...
      <div className="grid md:grid-cols-2 gap-12">
        {/* FORMULARIO */}
        <div className={`${mobileView === 'formulario' ? 'block' : 'hidden md:block'} space-y-4 bg-zinc-50 dark:bg-zinc-900 p-6 rounded-[2rem] sticky top-8 h-fit border border-zinc-100 dark:border-zinc-800 shadow-sm`}>
          <h2 className="text-[0.625rem] font-bold uppercase tracking-widest text-tekila-pink mb-2 flex items-center gap-2">
          ...
          <div className="space-y-4">
            ...
            <label className="flex items-center gap-2 text-[0.625rem] uppercase font-bold cursor-pointer bg-zinc-200 dark:bg-zinc-800 p-4 rounded-2xl justify-center italic">
            ...
            <button onClick={() => { cancelarEdicion(); setMobileView('lista'); }} className="w-full text-[0.625rem] uppercase font-bold text-zinc-400 text-center">Cancelar</button>
          </div>
        </div>

        {/* LISTADO TIPO ACORDEÓN */}
        <div className={`${mobileView === 'lista' ? 'block' : 'hidden md:block'} space-y-2`}>
          <h3 className="text-[0.625rem] uppercase tracking-[0.2em] font-bold text-zinc-400 px-2 mb-4 text-center md:text-left">Orden de los servicios</h3>
          ...
          {servicios.map((s: any, index: number) => {
            ...
            return (
              <div key={s.id}
                ...
                className={`overflow-hidden bg-white dark:bg-zinc-900 border transition-all duration-300 cursor-pointer ${isExpanded ? 'rounded-[2rem] border-tekila-pink shadow-lg' : 'rounded-[1.375rem] border-zinc-100 dark:border-zinc-800 shadow-sm'}`}
              >
                <div className="flex items-center justify-between p-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="overflow-hidden">
                      ...
                      <p className="text-[0.625rem] text-tekila-pink font-bold">{formatMoney(s.precio)}</p>
                    </div>
                  </div>
                ...
                <div className={`grid transition-all duration-300 ease-in-out ${isExpanded ? 'grid-rows-[1fr] opacity-100' : 'grid-rows-[0fr] opacity-0'}`}>
                  <div className="overflow-hidden">
                    <div className="p-5 pt-2 border-t border-zinc-50 dark:border-zinc-800 mt-2 bg-zinc-50/50 dark:bg-zinc-950/30">
                      <p className="text-[0.6875rem] text-zinc-500 dark:text-zinc-400 mb-5 leading-relaxed">
                        {s.descripcion || 'Sin descripción detallada.'}
                      </p>
                      <div className="flex gap-2">
                        <button
                          onClick={(e) => { e.stopPropagation(); prepararEdicion(s); setMobileView('formulario'); }}
                          className="flex-1 flex items-center justify-center gap-2 py-3.5 bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-2xl text-[0.625rem] font-black uppercase tracking-widest hover:border-tekila-pink transition-all shadow-sm"
                        >
