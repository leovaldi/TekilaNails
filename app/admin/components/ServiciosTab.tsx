      ...
      <div className="flex justify-center mb-6">
        <div className="flex bg-zinc-100 dark:bg-zinc-900 p-1 rounded-2xl w-full max-w-xs">
          <button 
            ...
            className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl text-[10px] uppercase font-bold tracking-widest transition-all ${mobileView === 'lista' ? 'bg-white dark:bg-zinc-800 text-tekila-pink shadow-sm' : 'text-zinc-400'}`}
          >
            <List size={14} /> Lista ({servicios.length})
          </button>
          <button 
            onClick={() => setMobileView('formulario')}
            className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl text-[10px] uppercase font-bold tracking-widest transition-all ${mobileView === 'formulario' ? 'bg-white dark:bg-zinc-800 text-tekila-pink shadow-sm' : 'text-zinc-400'}`}
          >
      ...
      <div className="grid md:grid-cols-2 gap-12">
        ...
        <div className={`${mobileView === 'formulario' ? 'block' : 'hidden md:block'} space-y-4 bg-zinc-50 dark:bg-zinc-900 p-6 rounded-[32px] sticky top-8 h-fit border border-zinc-100 dark:border-zinc-800 shadow-sm animate-in fade-in slide-in-from-bottom-2 duration-300`}>
          ...
          <div className="space-y-1">
            ...
            <input type="text" placeholder="Ej: Esculpidas" className={`w-full p-4 bg-white dark:bg-zinc-800 border ${errores.nombre ? 'border-red-500' : 'border-zinc-100 dark:border-zinc-700'} rounded-2xl text-sm outline-none focus:border-tekila-pink`} value={nuevoServicio.nombre} onChange={e => {setNuevoServicio({...nuevoServicio, nombre: e.target.value}); setErrores({...errores, nombre: false})}} />
          </div>
          ...
          <div className="space-y-1">
            ...
            <div className="relative">
              ...
              <input type="number" placeholder="0" className={`w-full p-4 pl-8 bg-white dark:bg-zinc-800 border ${errores.precio ? 'border-red-500' : 'border-zinc-100 dark:border-zinc-700'} rounded-2xl text-sm outline-none focus:border-tekila-pink`} value={nuevoServicio.precio} onChange={e => {setNuevoServicio({...nuevoServicio, precio: e.target.value}); setErrores({...errores, precio: false})}} />
            </div>
          </div>

          <div className="space-y-1">
            <label className="text-[9px] uppercase font-bold text-zinc-400 ml-1">Descripción</label>
            <textarea placeholder="¿Qué incluye?" className="w-full p-4 bg-white dark:bg-zinc-800 border border-zinc-100 dark:border-zinc-700 rounded-2xl text-sm h-24 outline-none focus:border-tekila-pink resize-none" value={nuevoServicio.descripcion} onChange={e => setNuevoServicio({...nuevoServicio, descripcion: e.target.value})} />
          </div>
        ...
        <div className={`${mobileView === 'lista' ? 'block' : 'hidden md:block'} space-y-3 animate-in fade-in slide-in-from-bottom-2 duration-300`}>
          ...
          {servicios.map((s: any) => (
            <div key={s.id} className="flex items-center justify-between p-4 bg-white dark:bg-zinc-900 border border-zinc-100 dark:border-zinc-800 rounded-[24px] shadow-sm hover:border-tekila-pink/30 transition-all group">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-tekila-pink font-bold">{formatMoney(s.precio)}</p>
                </div>
              </div>
