...
export async function POST(request: Request) {
  try {
    ...
    console.log(`[LOG] Iniciando procesamiento de reserva ID: ${reserva.id}`);
    ...
    if (!process.env.GOOGLE_REFRESH_TOKEN) {
      console.error("[TEKILA_ERROR] GOOGLE_REFRESH_TOKEN no definido en el entorno");
    }
    ...
    if (!fechaRaw) {
      console.error("[TEKILA_ERROR] Fecha no encontrada en la reserva");
    ...
    try {
      ...
      console.error("[TEKILA_ERROR] Falla al insertar en Google Calendar:", calErr);
    }

    // 4. Actualización y bloqueo en Supabase
    try {
      ...
      console.error("[TEKILA_ERROR] Error crítico al actualizar Supabase:", dbErr);
      throw new Error("Falla al actualizar estado en Supabase");
    }
          ...
          <div style="font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; background-color: #fafafa; padding: 40px 20px; color: #111;">
            <div style="max-width: 500px; margin: 0 auto; background-color: #ffffff; border-radius: 30px; overflow: hidden; box-shadow: 0 10px 30px rgba(0,0,0,0.05); border: 1px solid #f0f0f0;">
              ...
              <div style="padding: 40px 30px;">
                <h1 style="font-size: 22px; font-weight: 300; margin-bottom: 25px; border-bottom: 1px solid #f0f0f0; padding-bottom: 15px;">¡Nuevo turno confirmado!</h1>
            ...
            <p style="text-align: center; font-size: 10px; color: #d1d1d6; margin-top: 20px; text-transform: uppercase; letter-spacing: 2px;">© 2026 Tekila Nails • Maipú, Mendoza</p>
          </div>
        `
      });
    } catch (mailErr) {
      console.error("[TEKILA_ERROR] Falla al enviar Email:", mailErr);
    }

    // 6. Notificación por Telegram
    try {
      ...
      console.error("[TEKILA_ERROR] Falla al enviar Telegram:", tgErr);
    }

    return NextResponse.json({ success: true });

  } catch (error: any) {
    console.error("[TEKILA_FATAL_ERROR] Error general en el flujo:", error);
